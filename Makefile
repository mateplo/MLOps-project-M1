...
.PHONY: help init lock data validate train promote evaluate export all predict test lint format ui serve \
        ...
        compose-up compose-down compose-train compose-monitoring release-check export-onnx publish-model deploy-space build-multi clean
...
export-onnx:     ## Export artifacts/model.joblib to ONNX (+ preprocess.json) for in-browser inference
	$(PY) -m src.export_onnx --config $(CONFIG)

publish-model:   ## Publish artifacts/ (champion: joblib + onnx + meta) to the HF model repo (needs HF_TOKEN)
	$(PY) -m src.publish --config $(CONFIG)

deploy-space:    ## (Re)deploy the static HF Space (in-browser inference), needs HF_TOKEN
	$(PY) -m src.deploy_space --config $(CONFIG) $(if $(IMAGE_REF),--image $(IMAGE_REF),)
